-- ============================================================
-- SalonOS — Migração 16: baixa automática de estoque ao fechar comanda
-- Usa a receita (estabelecimento_atividade_produtos, migração 15) pra
-- descontar de produtos.quantidade_em_estoque na mesma hora em que
-- atendimentos/caixa_entradas são gravados -- comanda aberta é rascunho,
-- fechar é o "aconteceu de verdade" (mesmo princípio já usado no resto
-- da função). Não bloqueia o fechamento se o estoque ficar negativo --
-- vira alerta visual na tela de Estoque, não uma trava dura.
-- ============================================================
...
begin
  ...
  for v_item in select * from comanda_itens where comanda_id = p_comanda_id loop
    ...
    update produtos p set quantidade_em_estoque = quantidade_em_estoque - (r.quantidade_usada * v_item.quantidade)
    from estabelecimento_atividade_produtos r
    where r.produto_id = p.id and r.estabelecimento_atividade_id = v_item.estabelecimento_atividade_id;
  end loop;
